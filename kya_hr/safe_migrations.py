...
AFTER_MIGRATE: list[tuple[str, str]] = [
    ...
    ("kya_hr.maintenance.setup_equipe_effectifs.execute", "Effectifs d'équipe (Phase 2) : champs éditables par le chef sur la fiche Employee (rôle dans l'équipe / compétences / notes du chef) ; le journal Modification Info Employe KYA trace mutations + notifie l'employé (Phase 3)"),
    ("kya_hr.maintenance.setup_avenant.execute", "Avenant au contrat (Flux Avenant KYA : Brouillon → En attente DG → Signé) : promotion/mutation/rémunération, corps dynamique éditable par la RH, montants en lettres, accords en genre, signature DG estampillée OU en ligne"),
    ("kya_hr.maintenance.setup_stage_immersion.execute", "Contrat de stage d'immersion (Flux Stage Immersion KYA) : variantes Établissement/Tuteur légal, accords en genre, multi-signataires (stagiaire/maître/DG/garant), fallback élève sans email (RH imprime pour signature physique)"),
    ("kya_hr.maintenance.ensure_kya_feature_shortcuts.execute", "Accessibilité : raccourcis de TOUTES les fonctionnalités récentes dans le bon espace (Documents RH/Avenants/Immersion + Effectifs d'équipe côté RH & Direction & chef ; Catalogue articles côté Employés & Stock ; dashboards Direction). Additif, idempotent, enchaîne sync_workspace_shortcuts."),
